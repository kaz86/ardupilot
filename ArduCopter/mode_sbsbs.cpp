...
/*
 * Init and run calls for sbsbs flight mode
 */

// sbsbs_init - initialise loiter controller
bool ModeSbsbs::init(bool ignore_checks)
{
...
#if PRECISION_LANDING == ENABLED
bool ModeSbsbs::do_precision_loiter()
{
...
void ModeSbsbs::precision_loiter_xy()
{
    loiter_nav->clear_pilot_desired_acceleration();
...
void ModeSbsbs::run()
{
    ...
    static float target_climb_rate = 0.0f;

    target_yaw_rate=target_yaw_rate + 0.1;
    */
    ...
    static float target_euler_deg = 0.0f;

    static float now_time=0.0f; //sec
    static float action_time = 10.0f; //sec

    if( now_time >= 2*action_time){
        now_time=0.0f;   
    ...
    case AltHold_Flying:
        ...
        attitude_control->input_euler_angle_roll_pitch_yaw(target_roll, target_pitch, target_euler_deg, 1);
...
uint32_t ModeSbsbs::wp_distance() const
{
    return loiter_nav->get_distance_to_target();
}

int32_t ModeSbsbs::wp_bearing() const
{
